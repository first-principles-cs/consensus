...
raft_status_t raft_start_pre_vote(raft_node_t* node) {
    ...
    if (node->send_fn) {
        raft_pre_vote_t request = {
            ...
            .term = node->persistent.current_term + 1,  /* Would-be term */
...
raft_status_t raft_handle_pre_vote(raft_node_t* node,
    ...
    response->type = RAFT_MSG_PRE_VOTE_RESPONSE;
    response->term = node->persistent.current_term;
    response->vote_granted = false;

    /* Reject if we have a leader and haven't timed out */
    if (node->current_leader >= 0 && node->election_timer_ms < node->election_timeout_ms) {
        return RAFT_OK;
    }
...
raft_status_t raft_handle_pre_vote_response(raft_node_t* node,
    ...
    if (node->role != RAFT_PRE_CANDIDATE) return RAFT_OK;
    ...
    if (response->vote_granted && !node->votes_granted[from_node]) {
        ...
        /* Check for majority - if we have it, start real election */
        if (node->votes_received > node->num_nodes / 2) {
            return raft_start_election(node);
        }
    }
...
/* TimeoutNow handler (Phase 6 - leadership transfer) */
raft_status_t raft_handle_timeout_now(raft_node_t* node,
                                       const raft_timeout_now_t* request) {
    if (!node || !request) return RAFT_INVALID_ARG;
    if (!node->running) return RAFT_STOPPED;

    /* Only followers should respond to TimeoutNow */
    if (node->role != RAFT_FOLLOWER) return RAFT_OK;

    /* Verify the request is from current leader */
    if (request->term != node->persistent.current_term) return RAFT_OK;
    if (request->leader_id != node->current_leader) return RAFT_OK;

    /* Immediately start election (skip PreVote for leadership transfer) */
    return raft_start_election(node);
}
...
raft_status_t raft_handle_append_entries(raft_node_t* node,
    ...
    /* If we're a candidate or pre-candidate, step down to follower */
    if (node->role == RAFT_CANDIDATE || node->role == RAFT_PRE_CANDIDATE) {
        node->role = RAFT_FOLLOWER;
        node->votes_received = 0;
    }
...
raft_status_t raft_receive_message(raft_node_t* node, int32_t from_node,
    ...
    switch (type) {
        ...
        case RAFT_MSG_PRE_VOTE: {
            if (msg_len < sizeof(raft_pre_vote_t)) return RAFT_INVALID_ARG;
            raft_pre_vote_response_t response;
            raft_status_t status = raft_handle_pre_vote(node,
                (const raft_pre_vote_t*)msg, &response);
            if (status == RAFT_OK && node->send_fn) {
                node->send_fn(node, from_node, &response, sizeof(response),
                              node->user_data);
            }
        ...
        case RAFT_MSG_PRE_VOTE_RESPONSE: {
            if (msg_len < sizeof(raft_pre_vote_response_t)) return RAFT_INVALID_ARG;
            return raft_handle_pre_vote_response(node, from_node,
                (const raft_pre_vote_response_t*)msg);
        }

        case RAFT_MSG_TIMEOUT_NOW: {
            if (msg_len < sizeof(raft_timeout_now_t)) return RAFT_INVALID_ARG;
            return raft_handle_timeout_now(node, (const raft_timeout_now_t*)msg);
        }
